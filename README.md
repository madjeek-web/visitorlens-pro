...
## What is different from VisitorLens (v2)

| Point | VisitorLens v2 | VisitorLens Pro v3 |
|---|---|---|
| GeoIP services | 2 (1 primary + 1 fallback) | 4 services with automatic chain |
| Rate limits documented | Partially | Prominently in README and source |
| Minified file provided | No | Yes (14.6 KB, generated by build script) |
| CDN URL | No | Yes (jsDelivr) |
| Module formats | Plain script tag only | Script tag + CommonJS + ES Module (UMD) |
| ES module import | Not available | `import visitor from './visitor.esm.js'` |
| GeoIP tests | None (real HTTP requests) | Full mock suite, 100% offline |
| visitor.debug() on IE8 | Could throw silently | Each console method checked individually |
| Ad-blocker detection | Single DOM bait technique | 3-technique majority vote |
| visitor.geo_source | Not tracked | Tracks which service responded |
| All-failed callback | Polling only stops on geo_loaded | Stops on geo_loaded or all_failed |
